...
- **Feasibility spike (2026-07-06, VALIDATED against a live test team):** Channel threads are achievable with non-interactive `az` tokens, using a **two-token split**:
  ...
  - Real-time receive via Trouter (as the reference does for chats) is expected to extend to channels since chatsvc is the same backend, but is **not yet confirmed**; chatsvc **polling** is a proven receive fallback. Confirming Trouter-for-channels is a remaining planning task.
...
- **FR-004**: The system MUST require a Teams channel to be specified in settings before any agent can be brought online, provided as a **Teams channel deep-link URL** that the system parses into channel id + group/team id (as in `https://teams.microsoft.com/l/channel/<channelId>/...?groupId=<teamId>&tenantId=<...>`); the online action MUST be blocked with a clear prompt when none is configured or the URL cannot be parsed.
- **FR-005**: The system MUST run a background Teams monitoring service that maintains a real-time connection to Teams and receives new messages from the monitored channel(s).
...
- **FR-007**: The system MUST filter inbound messages so that only messages that (a) are not duplicates, (b) are not stale backlog, (c) are not the agent's own posts, (d) arrive in a thread bound to an online agent (thread-scoped routing; no @mention required), and (e) originate in the configured channel are dispatched. There is NO sender restriction — any channel member's message qualifies (see FR-007a).
- **FR-007a**: Because the agent posts its replies under the app user's own Teams identity (via Graph), the system MUST detect and exclude the agent's own posts by an embedded marker/prefix (not by sender identity) so a reply is never re-interpreted as a new inbound prompt (self-loop prevention).
...
- **Teams Monitor Settings**: user configuration for the feature. Attributes: channel deep-link URL (parsed into channel id + team/group id + tenant id), stop-command convention, check-in enabled + threshold + throttle interval. (No sender allow-list — any channel member may drive agents.)
- **Inbound Teams Message**: a received message under evaluation. Attributes: message id, conversation/thread id, sender identity, content, timestamp, matched bound-agent (via thread id).
...
- The target is a **true Teams channel** inside a Team; "thread" means a channel reply chain under a root post. **Feasibility validated by spike (2026-07-06)**: sending (root + replies) via Microsoft Graph and receiving via the internal chatsvc API both work with non-interactive `az` tokens (Graph token for send, ic3 token for receive). **Remaining dependency**: confirm real-time Trouter delivery for channels during planning; if unavailable, use chatsvc polling (proven) as the receive mechanism.
...
- The monitored scope for v1 is a single configured channel/conversation; monitoring many channels simultaneously is a later enhancement.
